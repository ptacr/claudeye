...
export function SubagentToolCard({ block, subagentEntries, projectName, sessionId }: SubagentToolCardProps) {
...
function EntryContent({ entry, allEntries, projectName, sessionId }: EntryRowProps): React.ReactNode {
  switch (entry.type) {
    case "user":
      return <UserContent entry={entry} />;
    case "assistant":
      return <AssistantContent entry={entry} allEntries={allEntries} projectName={projectName} sessionId={sessionId} />;
...
export const EntryRow = React.memo(function EntryRow({ entry, allEntries, projectName, sessionId }: EntryRowProps) {
  return (
    <div
    ...
    >
      ...
      <div className="px-4 py-3">
        <EntryContent entry={entry} allEntries={allEntries} projectName={projectName} sessionId={sessionId} />
      </div>
    </div>
  );
});
